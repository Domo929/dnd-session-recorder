> ⚠️ **NEEDS REWRITE (post upstream-alignment).** This design targets our old
> `/api/uploads` route and `Upload.path`, which upstream rewrote (status flow +
> `create-with-upload` + progress tracking). Azure Blob storage remains a
> fork-specific concern (not an upstream PR). Re-derive against the aligned
> upload flow. See `files/upstream-alignment-master-plan.md` Phase D.
